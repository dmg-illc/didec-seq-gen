...
def save_model(model, epoch, scores, metrics_dict, losses, optimizer, args, metric, timestamp, seed):
    ...
    file_name = 'model_pret_smdv_NWMT_' + str(seed) + '_' + metric + '_' + timestamp + '.pkl'
...
def validate(val_loader, decoder, criterion_ce, i2w, device, print_freq, word_map, current_epoch, break_flag, top_x, smoothing_method, print_flag):
    ...
    with torch.no_grad():
        for i, data in enumerate(val_loader):
            ...
            print('val i', i)
            imgs, caps, caplens, allcaps, masked_features = data
            ...
            scores, caps_sorted, decode_lengths, sort_ind = decoder(imgs, caps, caplens, masked_features)
...
if __name__ == '__main__':
    ...
    for epoch in range(epochs):
         ...
         for i, data in enumerate(training_loader):
            ...
            image_set, caption_texts, caption_lengths, masked_features = data
         ...
         metrics_dict, recent_ours_beam = eval_pret_sm_dv_NWMT.evaluate(beam_size, decoder, val_loader_beam, device, word_map, len(word_map), rev_word_map, nlgeval, 'val', metric_obj)
