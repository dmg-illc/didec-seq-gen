...
def save_model(model, epoch, scores, metrics_dict, losses, optimizer, args, metric, timestamp, seed):
    ...
    file_name = 'model_pret_2rnndv__NWMT' + str(seed) + '_' + metric + '_' + timestamp + '.pkl'
...
def validate(val_loader, decoder, criterion_ce, i2w, device, print_freq, word_map, current_epoch, break_flag, top_x, smoothing_method, print_flag):
    ...
    with torch.no_grad():
        for i, data in enumerate(val_loader):
            ...
            print('val i', i)
            imgs, caps, caplens, allcaps, word_fxs = data

            # Move to device, if available
            imgs = imgs.to(device)
            caps = caps.to(device)
            caplens = caplens.to(device)
            word_fxs = word_fxs.to(device)

            scores, caps_sorted, decode_lengths, sort_ind = decoder(imgs, caps, caplens, word_fxs)
...
if __name__ == '__main__':
    ...
    # add parts of the model that did not exist in the pretrained model
    ...
    for epoch in range(epochs):
         ...
         for i, data in enumerate(training_loader):
            ...
            image_set, caption_texts, caption_lengths, word_fxs = data
         ...
         metrics_dict, recent_ours_beam = eval_pret_2rnn_NWMT.evaluate(beam_size, decoder, val_loader_beam, device, word_map, len(word_map), rev_word_map, nlgeval, 'val', metric_obj)
