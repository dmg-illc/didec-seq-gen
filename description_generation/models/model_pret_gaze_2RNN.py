...
# to incorporate the DIDEC input into an additional Gaze LSTM
# that only takes features of an image masked with sequential fixation masks aligned with the utterances of words
...
class DecoderWithAttention(nn.Module):
    def __init__(self, vocab_size, embedding_dim, hidden_dim, attention_dim, features_dim, dropout, device):
        ...
        self.top_down_attention = nn.LSTMCell(self.hidden_dim + self.features_dim + self.embedding_dim, self.hidden_dim, bias=True) # top down attention LSTMCell
        self.fc = weight_norm(nn.Linear(self.hidden_dim, vocab_size))  # linear layer to find scores over vocabulary

        # Additional LSTM that only takes features of images masked with sequential fixation masks
        self.gaze_rnn = nn.LSTMCell(self.features_dim, self.hidden_dim, bias=True)
        self.hid2feat = nn.Linear(self.hidden_dim, self.features_dim)
    ...
    def forward(self, image_feats, segment, lengths, word_fxs):
        ...
        h3, c3 = self.init_hidden_state(batch_size)  # initialize the additional Gaze LSTM
        ...
        for t in range(max(decode_lengths)):
            ...
            # gaze LSTM takes as input the features of an image masked with sequential fixations that are aligned with words
            # being fed at the same timestep into the top-down attention LSTM

            h3, c3 = self.gaze_rnn(word_fxs[:batch_size_t, t, :], (h3[:batch_size_t], c3[:batch_size_t]))

            h3feat = self.hid2feat(h3)

            # hidden state of the gaze LSTM is incorporated into the top-down attention module

            h1, c1 = self.top_down_attention(
                torch.cat([h2[:batch_size_t], h3feat, embeds_words[:batch_size_t, t, :]], dim=1), (h1[:batch_size_t], c1[:batch_size_t]))
