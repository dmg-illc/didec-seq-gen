...
# Top-down attention module is fed features of an image masked with the aggregated saliency map of fixations
...
class Attention(nn.Module):
    ...
    def forward(self, image_features, decoder_hidden):
        ...
        # same, torch.equal(att1 + att2.unsqueeze(1), att1.expand(5,36,1024) + att2.unsqueeze(1))
...
class DecoderWithAttention(nn.Module):
    ...
    def forward(self, image_feats, segment, lengths, masked_feats):
        """
        ...
        @param masked_feats: features of images masked with their aggregated saliency maps
        """
        ...
        masked_feats = masked_feats[sorted_idx]
        ...
        for t in range(max(decode_lengths)):
            ...
            # features of images masked with their aggregated saliency maps
            # the same features are fed at each timestep

            h1, c1 = self.top_down_attention(
                torch.cat([h2[:batch_size_t], masked_feats[:batch_size_t], embeds_words[:batch_size_t, t, :]], dim=1), (h1[:batch_size_t], c1[:batch_size_t]))
