...
# Top-down attention module is fed the mean bottom-up features of the given image
...
class Attention(nn.Module):
    ...
    def forward(self, image_features, decoder_hidden):
        ...
        att_comb = self.full_att(self.dropout(self.relu(att1 + att2.unsqueeze(1))))
        att = att_comb.squeeze(2)  # (batch_size, 36)
...
class DecoderWithAttention(nn.Module):
    ...
    def forward(self, image_feats, segment, lengths):
        """
        ...
        # Flatten image
        image_feat_mean = image_feats.mean(1).to(self.device)  # (batch_size, num_pixels, encoder_dim)

        # pack sequence
        sorted_lengths, sorted_idx = torch.sort(lengths.squeeze(1), descending=True)

        image_feats = image_feats[sorted_idx]
        image_feat_mean = image_feat_mean[sorted_idx]
        ...
        for t in range(max(decode_lengths)):
            ...
            # no gaze input is fed into this model
            # top-dow attention LSTM receives the mean of the bottom-up image features
            # at each timestep, the same input is fed into the LSTM

            h1, c1 = self.top_down_attention(
                torch.cat([h2[:batch_size_t], image_feat_mean[:batch_size_t], embeds_words[:batch_size_t, t, :]],
