...
from models.model_pret_gaze_SM import DecoderWithAttention
...
def evaluate(beam_size, decoder, loader, device, word_map, vocab_size, rev_word_map, nlgeval, split, our_metric):
    ...
    with torch.no_grad():
        with open('smdv_pret_NWMT.txt', 'w') as hyp_file:

            # For each image
            for i, (image_features, caps, caplens, allcaps, masked_features) in enumerate(tqdm(loader, desc="EVALUATING AT BEAM SIZE " + str(beam_size))):
                ...
                masked_features = masked_features.expand(k, 2048)
                ...
                while True:
                    ...
                    h1, c1 = decoder.top_down_attention(
                        torch.cat([h2, masked_features, embeddings], dim=1),
                    ...
                    masked_features = masked_features[prev_word_inds[incomplete_inds]]
    ...
    with open('refs_metric_SM_NWMT.json', 'w') as f:
        json.dump(refs_metric, f)

    with open('hyps_metric_SM_NWMT.json', 'w') as f:
...
if __name__ == '__main__':
    ...
    # why do we need this? unless things are clashing in terms of the source codes
