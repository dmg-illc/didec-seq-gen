...
import eval_image_COCONL

from utils.util_funcs import AverageMeter, accuracy, adjust_learning_rate

def save_model(model, epoch, scores, metrics_dict, losses, optimizer, args, metric, timestamp):

    file_name = 'model_COCONLimage_' + metric + '_' + timestamp + '.pkl'
...
if __name__ == '__main__':
    ...
    parser.add_argument("-data_name", type=str, default='coconl')
    ...
    nlgeval = NLGEval() # evaluator for multiple metrics

    train_set = DidecDatasetCOCONL(data_folder, data_name, 'train', model_type)
    val_set = DidecDatasetCOCONL(data_folder, data_name, 'val', model_type)
    ...
    load_params_val = {'batch_size': batch_size, 'shuffle': False, 'num_workers': workers, 'pin_memory': True}
    ...
    for epoch in range(epochs):
         ...
         metrics_dict = eval_image_COCONL.evaluate(beam_size, decoder, val_loader_beam, device, word_map, len(word_map), rev_word_map, nlgeval, 'val')
         ...
         if not is_best:
             ...
             save_model(decoder, epoch, recent_metric, metrics_dict, losses, decoder_optimizer, args, metric, timestamp)
