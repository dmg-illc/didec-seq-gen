...
from models.model_pret_gaze_RNN import DecoderWithAttention

from utils.DidecDataset import DidecDataset
import eval_pret_rnn_NWMT
...
def save_model(model, epoch, scores, metrics_dict, losses, optimizer, args, metric, timestamp, seed):
    ...
    file_name = 'model_pret_rnndv_NWMT' + str(seed) + '_' + metric + '_' + timestamp + '.pkl'
...
if __name__ == '__main__':
    ...
    for epoch in range(epochs):
         ...
         metrics_dict, recent_ours_beam = eval_pret_rnn_NWMT.evaluate(beam_size, decoder, val_loader_beam, device, word_map, len(word_map), rev_word_map, nlgeval, 'val', metric_obj)
