...
# GENERATING SOME VISUAL EXAMPLES OF MASKS AND MASKED IMAGES
...
def get_masked_image_features(img_id, fixations, model, duration_weighted, img_transform, token_ind):
    ...
    saved_mask_name = 'example_mask_' + str(token_ind) + '.png'
    plt.savefig(saved_mask_name, bbox_inches = 'tight', pad_inches = 0)
    #plt.show()
    ...
    image_final_heatmap = Image.fromarray(final_heatmap * 255)
    image_final_heatmap = image_final_heatmap.crop((206, 50, 206 + 1267, 50 + 950))  # crop to the size of the actual image on the screen

    plt.figure(1)
    plt.axis('off')
    plt.imshow(image)
    saved_image_name = 'example_image_' + str(token_ind) + '.png'
    plt.savefig(saved_image_name, bbox_inches = 'tight', pad_inches = 0)
    #plt.show()


    plt.figure(1)
    plt.axis('off')
    plt.imshow(image_final_heatmap)
    saved_bwmask_name = 'example_bwmask_' + str(token_ind) + '.png'
    plt.savefig(saved_bwmask_name, bbox_inches = 'tight', pad_inches = 0)
    ...
    for img_id in alignments[ppn]:
        ...
        if img_id == '285766':
            ...
            for alg in single_alignment:
                ...
                masked_img_features = get_masked_image_features(img_id, fixations, resnet101, duration_weighted, img_transform, token_ind)
