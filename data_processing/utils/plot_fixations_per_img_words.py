...
#PLOTS ALL GAZE DATA (see plot_get_fix_windows.py to plot just the fixation windows etc)
...
# this is created in parse_gaze_data.py
...
for root, subdirs, files in os.walk(bordered_images_path):
    ...
    for f in files:
        ...
        mapped_file = '../data/alignments/cmu_mapped_' + str(ppn) + '_' + str(image_id) + '.txt'
        ...
        for l in range(len(alignment_lines)):
            ...
            if l == 0:
                ...
                split_alignment = alignment_per_item.split()  # tab space?
        ...
        for e in range(len(eyescan['timestamps'])):
            ...
            circle1 = plt.Circle((eye_x, eye_y), 10, color='r', alpha=0.5)
            ax.add_artist(circle1)

            current_token = alignment_map[current_token_index]

            print(current_timestamp,float(current_token[1]), float(current_token[2]))

            # CRUCIAL, they are longer than audio boundaries
            #8.366615 7.95 8.34

            if current_timestamp > float(current_token[1]) and current_timestamp <= float(current_token[2]):

                plt.title(current_token[0], color='b')
